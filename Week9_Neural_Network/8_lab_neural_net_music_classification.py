...
for lr in [0.0001, 0.001, 0.01, 0.1, 1]:
    ...
    train_to_accuracy_cb = TrainToAccuracy(threshold=0.95, patience=3)
    history=model.fit(Xtr_scale, ytr, epochs=500, batch_size=batch_size,
              validation_split=0.2, callbacks=[train_to_accuracy_cb], verbose=0)
...
learning_rates = [d['learning_rate'] for d in metrics_vs_lr]
times = [d['train_time'] for d in metrics_vs_lr]
energies = [d.get('total_energy', None) for d in metrics_vs_lr]  # total_energy might not be present for CPU runtime
accuracies = [d['test_accuracy'] for d in metrics_vs_lr]
epochs = [d['epochs'] for d in metrics_vs_lr]

# Determine if GPU or CPU runtime based on the presence of 'total_energy' data
gpu_runtime = any(energies)

# Creating subplots
if gpu_runtime:
    # GPU runtime: 4 subplots
    fig, axs = plt.subplots(2, 2, figsize=(14, 10))
    axs[0, 0].bar(learning_rates, times)
    axs[0, 0].set_title('Time to Accuracy')
    axs[0, 0].set_xlabel('Learning Rate')
    axs[0, 0].set_ylabel('Time (s)')

    axs[0, 1].bar(learning_rates, energies)
    axs[0, 1].set_title('Energy to Accuracy')
    axs[0, 1].set_xlabel('Learning Rate')
    axs[0, 1].set_ylabel('Energy (Units)')

    axs[1, 0].bar(learning_rates, accuracies)
    axs[1, 0].set_title('Test Accuracy')
    axs[1, 0].set_xlabel('Learning Rate')
    axs[1, 0].set_ylabel('Accuracy')

    axs[1, 1].bar(learning_rates, epochs)
    axs[1, 1].set_title('Epochs')
    axs[1, 1].set_xlabel('Learning Rate')
    axs[1, 1].set_ylabel('Epochs')

else:
    # CPU runtime: 3 subplots
    fig, axs = plt.subplots(1, 3, figsize=(18, 6))
    axs[0].bar(learning_rates, times)
    axs[0].set_title('Time to Accuracy')
    axs[0].set_xlabel('Learning Rate')
    axs[0].set_ylabel('Time (s)')

    axs[1].bar(learning_rates, accuracies)
    axs[1].set_title('Test Accuracy')
    axs[1].set_xlabel('Learning Rate')
    axs[1].set_ylabel('Accuracy')

    axs[2].bar(learning_rates, epochs)
